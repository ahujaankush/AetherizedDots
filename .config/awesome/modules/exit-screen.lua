...
local create_button = function(symbol, hover_color, text, command)
    ...
    local hover_animation = animation:new({
        pos = helpers.color.hex_to_rgb(beautiful.lighter_black),
        easing = animation.easing.linear,
        duration = 0.2,
        update = function(self, pos)
            icon.markup = helpers.ui.colorize_text(icon.text, helpers.color.rgb_to_hex(pos))
            button.border_color = helpers.color.rgb_to_hex(pos)
        end,
    })
    ...
    button:connect_signal("mouse::enter", function()
        ...
        hover_animation:set(helpers.color.hex_to_rgb(hover_color))
    end)
    button:connect_signal("mouse::leave", function()
        ...
        hover_animation:set(helpers.color.hex_to_rgb(beautiful.lighter_black))
    end)
