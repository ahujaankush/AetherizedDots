...
local normal_color = colorMod.color({ hex = beautiful.exit_screen_fg })
...
local create_button = function(symbol, hover_color, text, command)
	...
	local transition = colorMod.transition(normal_color, colorMod.color({ hex = hover_color }))
	...
	local transitionFunc = rubato.timed({
		...
		subscribed = function(pos)
			icon.markup = helpers.colorize_text(icon.text, transition(pos).hex)
			button.border_color = transition(pos).hex
		end,
	})
...
local suspend = create_button(suspend_text_icon, x.color11, "Suspend", suspend_command)
local exit = create_button(exit_text_icon, x.color12, "Exit", exit_command)
local lock = create_button(lock_text_icon, x.color13, "Lock", lock_command)
