...
local builder = require("ui.widgets.control_center.notif-center.build-notifbox.notifbox-ui-elements")
local notifbox_core = require("ui.widgets.control_center.notif-center.build-notifbox")
...
local notifbox_box = function(notif, icon, title, message, app, _)
	...
	local notifbox_template = wibox.widget({
		...
		forced_height = #notif.actions > 0 and dpi(142) or dpi(112),
		bg = x.color0,
